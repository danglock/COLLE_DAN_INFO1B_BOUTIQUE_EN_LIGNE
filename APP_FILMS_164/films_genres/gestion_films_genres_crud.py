...
@app.route("/types_produits_afficher/<int:id_film_sel>", methods=['GET', 'POST'])
def films_genres_afficher(id_film_sel):
    print(" films_genres_afficher id_film_sel ", id_film_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_type, nom_type, type_description FROM t_types_entites ORDER BY id_type ASC"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """/* WHERE id_type = %(value_id_film_selected)s */"""
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    flash("""La table "t_types_entites" est vide. !""", "warning")
                    ...
                    flash(f"Données types de produits affichés !!", "success")
...
@app.route("/produits_afficher/<int:id_film_sel>", methods=['GET', 'POST'])
def produits_afficher(id_film_sel):
    print(" films_genres_afficher id_film_sel ", id_film_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_entite, nom_entite, prix_entite, description_entite, quantite_entite FROM t_entites ORDER BY id_entite ASC"""
                if id_film_sel == 0:
                    # le paramètre 0 permet d'afficher tous les films
                    # Sinon le paramètre représente la valeur de l'id du film
                    mc_afficher.execute(strsql_genres_films_afficher_data)
                    ...
                    # En MySql l'instruction HAVING fonctionne comme un WHERE... mais doit être associée à un GROUP BY
                    # L'opérateur += permet de concaténer une nouvelle valeur à la valeur de gauche préalablement définie.
                    strsql_genres_films_afficher_data += """/* WHERE id_entite= %(value_id_film_selected)s */"""
                ...
                # Différencier les messages.
                if not data_genres_films_afficher and id_film_sel == 0:
                    flash("""La table "t_entites" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_film_sel > 0:
                    # Si l'utilisateur change l'id_film dans l'URL et qu'il ne correspond à aucun film
                    flash(f"Le film {id_film_sel} demandé n'existe pas !!", "warning")
                else:
    ...
    print("films_genres_afficher  ", data_genres_films_afficher)
    # Envoie la page "HTML" au serveur.
    return render_template("produits/produits_afficher.html", data=data_genres_films_afficher)
